...
export const en: TranslationTree = {
	...
	settings: {
		...
		features: {
			...
			timeEntriesStorage: {
				name: "Time entries storage location",
				description: "Choose where time data is stored. 'Task file' stores entries in each task's frontmatter. 'Daily note' stores entries in daily note frontmatter with task links.",
				taskFile: "Task file",
				dailyNote: "Daily note",
			},
			nlpDateTimeInput: {
				name: "Natural language date/time input",
				description: "Use text fields with natural language parsing (e.g., '9am tomorrow', '2/18 3pm') instead of native date pickers in time entry modals.",
			},
		},
		defaults: {
			header: {
	...
	modals: {
		...
		timeInfo: {
			titleTimeBlock: "Time Block",
			titleTimeEntry: "Time Entry",
			dateTime: "Date & Time",
			date: "Date",
			...
			taskRequired: "A task must be selected",
			timeRequired: "Start and end times are required",
			quickDates: "Quick dates",
		},
		timeMigration: {
			title: "Migrate Time Data",
			toTaskDescription: "This will move all task-linked time entries from daily notes to task files.",
			toDailyNoteDescription: "This will move all time entries from task files to daily notes.",
			backupWarning: "We recommend backing up your vault before proceeding.",
			countLabel: "Found {entries} entries across {sources} files.",
			proceed: "Proceed",
			cancel: "Cancel",
		...
		taskEdit: {
			...
			sections: {
				...
				totalTracked: "Total tracked: {time}",
				noTitle: "(no title)",
				running: "running",
			},
			metadata: {
