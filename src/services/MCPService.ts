	...
	private registerTaskTools(server: McpServer): void {
		...
		tool(
			...
			"Create a new task",
			{
				...
				next_scheduled: z.string().optional().describe("Scheduled date (YYYY-MM-DD)"),
		...
		tool(
			...
			"Update an existing task's properties",
			{
				...
				next_scheduled: z.string().nullable().optional().describe("New scheduled date (YYYY-MM-DD) or null to clear"),
			...
			async ({ text }: any) => {
				try {
					...
					const taskData: TaskCreationData = {
						...
						due: parsed.dueDate,
						next_scheduled: parsed.scheduledDate,
						tags: parsed.tags,
